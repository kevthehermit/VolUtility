               if (postOptions['reset']){
                ...
                $.each(child_keys, function( index, value ) {
                    ...
                    if ( $("#"+parent_key ).length > 0) {
                        ...
                        console.log('Here');
                        $("#"+parent_key+"_Children").append("<li id='"+key_id+"'><input type=\"checkbox\" checked=\"checked\" id=\"item-"+index+"\" /><label for=\"item-"+index+"\" onclick=\"ajaxHandler('HiveViewer', {'file_id':'"+file_id+"', 'key': '"+encodeURIComponent(value)+"', 'reset': false, 'target_div':'none'}, false )\">"+value+"</label>");
                    ...
                    // Else create new node
                        ...
                        $('#nodelist ul').append("<li id='"+key_id+"'><input type=\"checkbox\" checked=\"checked\" id=\"item-"+index+"\" /><label for=\"item-"+index+"\" onclick=\"ajaxHandler('HiveViewer', {'file_id':'"+file_id+"', 'key': '"+encodeURIComponent(value)+"', 'reset': false, 'target_div':'none'}, false )\">"+value+"</label>");
                    }
